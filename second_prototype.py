# -*- coding: utf-8 -*-
"""
Created on Wed Aug 18 20:38:24 2021
...
#open dataset to store where survey data was stored to append new users
    with open("second_it.txt", "a") as fo:
...
def login():
    ...
    while notloggedin==True:
        with open("second_it.txt", "r") as f:
...
# IN THE NEW ITERATION THE ability to iterate through eeach index was added enabling each user get their 
#index number printed, which is i.e. its registration number
#furthermore, as seen in the first User Testing, now users can wave and the number of waved users will be updated
# this allow users to track how their matching is going and whether they have done too much and have non reciprocal responses
def profile(username):
    print()
    print()
    print("Username:", username)
    print(" --Welcome to your profile-- ")
    with open("second_it.txt", newline="") as f:
        ...
        for idx, row in temporarylist:
            for field in row:
                if field==username:
                    ...
                    wavedcount=int(reader[username_index][15])
    print("Total recommendations you have waved :", wavedcount)
    waved=int(input("How many additional possible matches did you wave today? : "))
    wavedcount=wavedcount+waved
    print("Potential Matches you have waved :", wavedcount)

    #New Feature - add ability to waved count in file
    temporarylist=[]
    updatedlist=[]

    with open("second_it.txt", newline="") as f:
        reader=list(csv.reader(f))
        temporarylist=reader #storing a copy of file contens into tempp list
        for row in reader:
            for field in row:
                if field==username:
                    updatedlist.append(row)
                    updatedlist[0][15]=int(updatedlist[0][15])+waved #takes initial value and add counts 
        updatecontactcount(updatedlist,temporarylist)#calling a subroutine

def updatecontactcount(updatedlist, temporarylist):
    #editing value in file
    for index, row in enumerate(temporarylist):
        for field in row:
            if field==updatedlist[0]:
                temporarylist[index]=updatedlist # replacing old records with updated records

    with open("second_it.txt", "w", newline="") as f:
        writer=csv.writer(f)
        writer.writerows(temporarylist)
        print("file has been updated")
        print("people you have waved at:",updatedlist[0][15])
    print("----what´s next?")
